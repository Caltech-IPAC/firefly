...
my (%browsers, %searches, %searches_nodata, %searches_by_originator, %downloadByWavelength, %downloadByDataType, %downloadFileByExt, %searchHosts, %downloadHosts, %visuCallsByType);
...
for my $f ( 0 .. $#ARGV ) {
	...
	my @parts = split/ -- /;
	if ($#parts > 0 or /\s+visu\s+/) {
	    s/127.0.0.1, 127.0.0.1/127\.0\.0\.1/;
	    my ($date, $dtime, $category, $subcategory, $ip, $time) = split('\s+', $parts[0]);
#     printf "(category=%s subcategory=%s ip=%s, time=%s)\n", $category, $subcategory, $ip, $time;
	    if ($category eq "info" and $subcategory eq "client") {
		my ($browser, $version, $os) = m/b\:(\w+) \| v\:(.+) \| os\:(.+)( \| |$)/;
		my $browser_entry = $os . " " . $browser . " " . $version;
#     printf "browser=%s version=%s os=%s\n", $browser, $version, $os;
		if (not defined($browsers{$browser_entry})) {
		    $browsers{$browser_entry} = 0;
		}
		$browsers{$browser_entry} += 1;
	    } elsif ($category eq "visu") {
		my $type = $subcategory;
		if (not defined($visuCallsByType{$type})) {
		    $visuCallsByType{$type} = 1;
		} else {
		    $visuCallsByType{$type}++;
		}
	    } elsif ($category eq "search") {
		my ($rows, $sizeMB) = m/rows\:(\d+) \| fsize\(MB\)\:(.+) \| from:/;
		my $type = $subcategory;
		if ($rows < 1) {
		    $nodata_present = 1;
		    ...
		    ($searches_nodata{$type})->addLine($time, $sizeMB) ;
		} else {
		    if (not defined($searches{$type})) {
			$searches{$type} = new Stat();;
		    }
		    ($searches{$type})->addLine($time, $sizeMB) ;
		}
		# some searches might have originator set, ex. Originator=HTTP_ByFixedTarget for SHA VO searches 
		my ($originator) = ($parts[1] =~ m/Originator=([^,]+)[,\)]/);
		if (defined ($originator)) {
		    if (not defined($searches_by_originator{$originator})) {
			$searches_by_originator{$originator} = new Stat();;
		    }
		    ($searches_by_originator{$originator})->addLine($time, $sizeMB) ;
		}
		if (not defined $searchHosts{$ip}) {
		    $searchHosts{$ip} = 0;
		}
		$searchHosts{$ip} +=1;
	    } elsif ($category eq "dl") {
		#size zipped (uncompressed size of all files) and time it took to zip
		if ($subcategory eq "file") {
		    my ($sizeMB, $ext) = m/size\(MB\)\:(.+) \| u\:.+ \| file\:.*\.([^\.]*)$/;
		    $download_file_stat->add(1, $sizeMB);
		    if (not defined($downloadFileByExt{$ext})) {
			$downloadFileByExt{$ext} = new MiniStat();
		    }
		    $downloadFileByExt{$ext}->add(1, $sizeMB);
		} elsif ($subcategory eq "package") {
		    my ($type, $wtime, $sizeMB) = m/type\:(.+) \| zipT.* \| waitT\(sec\)\:(.+) \| size\(MB\)\:(.+) \|/;
		    if ($type eq "immediate") {
			$download_immediate_stat->addLine($wtime, $sizeMB);
		    } elsif ($type eq "queued") {
			if ($sizeMB > 0) {
			    $download_queued_stat->addLine($wtime, $sizeMB);
			} else {
		    ...
		    $downloadHosts{$ip} +=1;
		}  elsif ($subcategory eq "wavelength") {
		    my @fields = split(/ \| /, $parts[1]);
		    if ($#fields >= 1) {
			for my $f ( 1 .. $#fields) {
			    my ($type, $files, $sizeB) = ($fields[$f] =~ m/(.+)\:\s(\d+),(\d+)/);
			    if (not defined($downloadByWavelength{$type})) {
				$downloadByWavelength{$type} = new MiniStat();
			    }
			    $downloadByWavelength{$type}->add($files, $sizeB/1024.0/1024.0);
			    $download_by_wavelength_total->add($files, $sizeB/1024.0/1024.0);
			}
		    }
		} elsif ($subcategory eq "datatype") {
		    my @fields = split(/ \| /, $parts[1]);
		    if ($#fields >= 1) {
			for my $f ( 1 .. $#fields) {
			    my ($type, $files, $sizeB) = ($fields[$f] =~ m/(.+)\:\s(\d+),(\d+)/);
			    ...
			    $downloadByDataType{$type}->add($files, $sizeB/1024.0/1024.0);
			    $download_by_datatype_total->add($files, $sizeB/1024.0/1024.0);
			}
		    }
		}
	    }
...
if (scalar(keys(%visuCallsByType)) > 0) {
    printf "\nVISIBILITY FUNCTION CALLS: \n\n";

    printf STDOUT "%10s %10s\n", "Function", "Calls";
    printf STDOUT "%10s %10s\n", "--------", "-----";
    foreach my $type (sort { if ($visuCallsByType{$a} < $visuCallsByType{$b}) {return 1;} else {return -1;} } keys %visuCallsByType) {
	printf STDOUT "%10s %10d\n", $type, $visuCallsByType{$type};
    }
}
...
DESCRIPTION
    ...
    -v             -- verbose 
                      ...
                      3 complete report with the details for each IP address
...
Search Type          Total Searches   Average Time        min          max       Average Size          min         max
...
format REC =
@<<<<<<<<<<<<<<<<<<<<   @#######      @######.###  @######.###  @######.###       @######.###  @######.###  @######.### 
$search_type, $rec->getTotalNum(), $rec->getAverageTime(), $rec->getMinTime(), $rec->getMaxTime(), $rec->getAverageSize(), $rec->getMinSize(), $rec->getMaxSize()
.

format TOP_NODATA =

Search Type          Total Searches   Average Time        min          max  
                                             (sec)       (sec)        (sec) 
-----------------    --------------   --------------------------------------
.
...
format REC_NODATA =
@<<<<<<<<<<<<<<<<<<<<   @#######      @######.###  @######.###  @######.### 
$search_type, $rec->getTotalNum(), $rec->getAverageTime(), $rec->getMinTime(), $rec->getMaxTime()
.
