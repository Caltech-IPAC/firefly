...
# info to be included in IPAC Table header 
    ...
    if (defined $name) {
        ...
        if ($name eq "5MUSES") {
            ...
            $self->{description} = "CLASH: The Cluster Lensing And Supernova survey with Hubble";
        } elsif ($name eq "Cygnus-X") {
            $self->{description} = "Cygnus-X: A Spitzer Legacy Survey of the Cygnus-X Complex";
        } elsif ($name eq "FEPS") {
            ...
            $self->{description} = "Spitzer FLS: Extragalactic First Look Survey";
        } elsif ($name eq "FLS_ELAISN1_R") {
            $self->{description} = "Spitzer FLS: NOAO ELAIS N1 R-band Data";
        } elsif ($name eq "FLS_HECTOSPEC") {
            $self->{description} = "Spitzer FLS: Ancillary MMT/Hectospec Spectral Data";
            ...
            $self->{description} = "SASS: Spitzer Atlas of Stellar Spectra";
        } elsif ($name eq "SDWFS") {
            $self->{description} = "SDWFS: Spitzer Deep, Wide-Field Survey";            
        } elsif ($name eq "SEP") {
            $self->{description} = "SEP: Spitzer MIPS 24 and 70 micron Imaging of the South Ecliptic Pole";
        } elsif ($name eq "SERVS") {
            $self->{description} = "SERVS: Spitzer Extragalactic Representative Volume Survey";
            ...
            $self->{description} = "SSDF: SPT-Spitzer Deep Field";
        } elsif ($name eq "SSGSS") {
