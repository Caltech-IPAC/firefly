...
# by default do not remove primary suffix to match primary products with ancillary
    ...
    if ($filepath =~ /Abell1763\//) {
	    $self->{primary} = qr/(\d|WIRC_?.|A1763_r)$/;
	    $self->{coverage} = qr/cov$/;
	    $self->{uncertainty} = qr/unc|weight$/;
    } elsif ($filepath =~ /C2D\/images_Av.tbl/) {
	    $self->{primary} = qr/_Av$/;
	    ...
	    $self->{uncertainty} = qr/(unc|std|mosaic_unc)/;
        ...
        $self->{primary} = qr/(phot.1|phot.2)$/;
        #$self->{primarySuffixToRemove} = qr/phot$/;
        $self->{coverage} = qr/wt$/;
        $self->{mask} = qr/final_mask$/;
