...
public class RandomGroupsHDU extends BasicHDU {
    ...
    public static Data manufactureData(Header hdr) throws FitsException {
        ...
        for (int i = 0; i < gcount; i += 1) {
            ((Object[][]) dataArray)[i][0] =
                    ((Object[]) edu.caltech.ipac.visualize.fits.util.ArrayFuncs.deepClone(sampleRow))[0];
            ((Object[][]) dataArray)[i][1] =
                    ((Object[]) edu.caltech.ipac.visualize.fits.util.ArrayFuncs.deepClone(sampleRow))[1];
        }
    ...
    public void info() {
        ...
        if (data == null || data.length < 1 || data[0].length != 2) {
            ...
            System.out.println("    Parameters: " + edu.caltech.ipac.visualize.fits.util.ArrayFuncs.arrayDescription(data[0][0]));
            System.out.println("    Data:" + edu.caltech.ipac.visualize.fits.util.ArrayFuncs.arrayDescription(data[0][1]));
        }
    }
}
