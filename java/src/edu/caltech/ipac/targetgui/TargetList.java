package edu.caltech.ipac.targetgui;
...
public class TargetList implements Serializable,
    ...
    public void setSelectedIndexArray(int selectedIdxAry[]) {
        int oldval[]= _selectedIndexes;
        if (selectedIdxAry!=null) {
            _selectedIndexes= new int[selectedIdxAry.length];
            System.arraycopy(selectedIdxAry,0, _selectedIndexes, 0,
                             selectedIdxAry.length);
        }
        else {
            _selectedIndexes= null;
        }
        if (!_doingBulkUpdates) {
            _propChange.firePropertyChange(SELECTED_INDEX_ARY,
                                           oldval, _selectedIndexes);
        }
    }
    ...
    public int [] getSelectedIndexArray() {
        ...
        if (_selectedIndexes!=null) {
            retval= new int[_selectedIndexes.length];
            System.arraycopy(_selectedIndexes,0, retval, 0,
