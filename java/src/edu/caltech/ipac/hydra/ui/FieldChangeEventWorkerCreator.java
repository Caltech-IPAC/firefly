...
public class FieldChangeEventWorkerCreator implements FormEventWorkerCreator {
    public FormEventWorker create(Map<String, String> params) {
        String fieldName = params.get("fieldName");

        boolean sync = false;
        String syncStr = params.get("sync");
        if (!StringUtils.isEmpty(syncStr)) {
            sync = Boolean.parseBoolean(syncStr);
        }

        return new FieldChangeEventWorker(fieldName, sync);
    }}
