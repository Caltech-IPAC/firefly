...
import edu.caltech.ipac.frontpage.data.DataSetDesc;
...
public class DataSetPanel {
    ...
    public DataSetPanel(String id) {
        makeUI(id, getDataSetDesc());

    }



    private void makeUI(String id, List<DataSetDesc> dsList) {

        RootPanel root= FFToolEnv.getRootPanel(id);
        HorizontalPanel hp= new HorizontalPanel();
        root.add(hp);
        Iterator<DataSetDesc> iterator= dsList.iterator();
        int i=0;

        for(; (i<MAX_PANELS && iterator.hasNext()); i++) {

            DataSetDesc d= iterator.next();
