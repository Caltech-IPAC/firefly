...
public class FileUtil 
{
  ...
  public static String getBase(File f) {
    ...
    // untested
    public static String convertByteToProximation(long size, ConvertTo to) {
        String retval= "0";
        if (to==ConvertTo.K) {
            retval= ((size / K) + 1) + "";
        }
        else if (to==ConvertTo.MEG) {
            long megs = size / MEG;
            long remain= size % MEG;
            long decimal = remain / MEG_TENTH;
            retval= megs +"."+ decimal;
    ...
    /**
     * Get just the name of a filename without the path.
     *
     * @param path a path such as <code>/home/data/a.dat</code>
     * @return String the filename only. i.e. if "/home/data/abc.dat" is
    ...
    public static String stripPath(String path) {
        ...
        char slash;
        if ((OSInfo.getOSName().indexOf("Win")) > -1 ||
                (OSInfo.getOSName().indexOf("win") > -1)) {
            slash = '\\';
        }
        else {
            slash = '/';
        ...
        if (i == -1 || i == 0) {
            ...
            base = path.substring(i + 1, path.length());
        }
        return base;
    }
     ...
     * Get just the name of a filename without the path.
     *
     * @param inFile a file such as <code>/home/data/a.dat</code>
     * @return File the filename only. i.e. if "/home/data/abc.dat" is
     *         passed to this method it will return "abc.dat"      
     */
    public static File stripPath(File inFile) {
        ...
        int i = inFile.getPath().lastIndexOf(slash);
        if (i == -1 || i == 0) {
            base = inFile.getPath();
        }
        else {
            base = inFile.getPath().substring(i + 1, inFile.getPath().length());
        }
    ...
    /**
     * launch a file browser to show a file directory on desktop
     *
     * @param dir is a file directory to be launched on desktop
     * @throws IOException throws if failed to open specific directory
     */
    public static void showDirectoryOnDesktop(File dir) throws IOException {
        if (dir!=null && dir.isDirectory() && dir.exists()) {
            String pathname = dir.getAbsoluteFile().toString();
            String command;
            ...
            String browser = findFileBroswer();
            if (browser != null)
                command = browser;
            else
                throw (new IOException("showDirectoryOnDesktop(): Can't find validfile browser!"));

            Runtime.getRuntime().exec(new String[] {command.trim(), pathname.trim()}, null, null);
        } else {
            throw (new IOException("showDirectoryOnDesktop(): Directory is invalid to open!"));
        }
    }

    /**
     * Find a file browser available at the current platform.
     *
     * @return a string of file browser available at the current platform.
     * @throws IOException if failed to open specific directory
     */
    public static String findFileBroswer() throws IOException{
        String retval = null;
        String[] fileBrowsers = {"nautilus", "konqueror", "firefox", "opera",
        ...
        try {
            if ( OSInfo.isPlatform(OSInfo.ANY_WINDOWS)){
                ...
                retval = "open";
            } else if ( OSInfo.isPlatform(OSInfo.SUN) || OSInfo.isPlatform(OSInfo.LINUX) ||
                    OSInfo.isPlatform(OSInfo.ANY_UNIX)) {
                for (String browser: fileBrowsers) {
