...
//	    out_x_pixel_size + "  out_y_pixel_size = " + out_y_pixel_size);
    ...
    private void set_out_from_in() {
        out_naxis1 = in_naxis1;
        out_naxis2 = in_naxis2;
        out_crval1 = in_crval1;
        out_crval2 = in_crval2;
        out_cdelt1 = in_cdelt1;
        out_cdelt2 = in_cdelt2;
        out_crpix1 = in_crpix1;
        out_crpix2 = in_crpix2;
        out_crota1 = in_crota1;
        out_crota2 = in_crota2;
...
/*  and do min-max on values in struct mm */
    ...
    private mmxy getImageBorder() {
        ...
        while (!findXLower && xLower < naxis1) {
            for (int i = 0; i < naxis2; i++) {
                if (!Float.isNaN(inData[i][xLower-1])) {
                    findXLower = true;
                    border.min_x = xLower;
                    break;
                }
        ...
        boolean findXUpper = false;
        int xUpper = naxis1 ;
        while (!findXUpper && xUpper > 0) {

            for (int i = 0; i < naxis2; i++) {
                if (!Float.isNaN(inData[i][xUpper-1])) {
                    findXUpper = true;
                    border.max_x = xUpper;
                    break;
                }
        ...
        int yLower = 1;
        boolean findYLower = false;
        while (!findYLower && yLower < naxis2) {

            for (int i = 0; i < naxis1; i++) {
                if (!Float.isNaN(inData[yLower-1][i])) {
                    findYLower = true;
                    border.min_y = yLower;
                    break;
                }
        ...
        boolean findYUpper = false;
        int yUpper = naxis2 ;
        while (!findYUpper && yUpper > 0) {

            for (int i = 0; i < naxis1; i++) {
                if (!Float.isNaN(inData[yUpper-1][i])) {
                    findYUpper = true;
                    border.max_y = yUpper;
                    break;
                }
    ...
    private void override_naxis_and_crpix() throws FitsException {
        ...
        minmax_xy(border.min_x, ( border.max_y + 1.0) / 2.0, mm);
        minmax_xy(border.min_x,  border.max_y, mm);
        minmax_xy((border.max_x + 1.0) / 2.0, border.min_y, mm);
        minmax_xy((border.max_x + 1.0) / 2.0, ( border.max_y + 1.0) / 2.0, mm);
        minmax_xy((border.max_x + 1.0) / 2.0, border.max_y, mm);
        minmax_xy(border.max_x, border.min_y, mm);
        minmax_xy( border.max_x, (border.max_y + 1.0) / 2.0, mm);
        minmax_xy(border.max_x, border.max_y, mm);
