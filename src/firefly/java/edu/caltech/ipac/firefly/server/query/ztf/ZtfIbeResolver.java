 ...
 */
public class ZtfIbeResolver {

    private static final Logger.LoggerImpl log = Logger.getLogger();

    public final static String ZTF_IBE_HOST = AppProperties.getProperty("ztf.ibe.host", "https://irsa.ipac.caltech.edu/ibe");
    public static boolean isTestMode = false;

    public ZtfIbeResolver() {
    }
    ...
    /**
     * Gets value of the column from ZTF PIDs
     *
     * @param pid     pids
    ...
    private static URL createURL(long[] pid) throws MalformedURLException {
        String url = QueryUtil.makeUrlBase(ZTF_IBE_HOST) + "/search/ztf/images/level1?where=pid%20in%20(";
    ...
    protected File getTempFile() throws IOException {
        File f = File.createTempFile("ztfFiles-", ".tbl", (isTestMode ? new File(".") : ServerContext.getTempWorkDir()));
        if (isTestMode) {
            f.deleteOnExit();
        }
        return f;
