...
import edu.caltech.ipac.firefly.visualize.FootprintAsMarkers;
...
/**
 * The command handles the adding of a circular marker - another command should
 * be implemented for the footprint
 * 
 * @author ejoliet
 *
 */
public class FootprintToolCmd extends BaseGroupVisCmd implements WebEventListener/* , PrintableOverlay */ {
	...
	public static final String CommandName = "FootprintTool"; // FIXME: should
																// be defined
																// elsewhere -
																// needed for
																// rpc, right?
	private HashMap<OverlayMarker, MarkerDrawing> _markerMap = new HashMap<OverlayMarker, MarkerDrawing>(10);
	private OverlayMarker _activeMarker = null; // multi shapes behaves as a
												// single marker

	private Mode _mode;
	private boolean _mouseDown = false;
	private boolean _doMove = false;
	private final WebPlotView.MouseInfo _mouseInfo = new WebPlotView.MouseInfo(new Mouse(), "Create a marker");
	...
	private void changeMode(Mode newMode) {
		...
		case OFF:
			...
			break;
		default:
			WebAssert.argTst(false, _mode.name()
					+ " current mode but changeMode(mode) only support for SelectType of ADD_FOOTPRINT or MOVE");
			break;
		}
	}
	...
	private void disableSelection() {
		...
		case OFF: /* do nothing */
			break;
		default:
			WebAssert.argTst(false, "disableSelection() only support for SelectType of ADD_FOOTPRINT or MOVE");
			break;
		}
	}
	...
	private void drag(WebPlotView pv, ScreenPt spt) {
		...
		case MOVE:
		 ...
		 case ROTATE:
			 ...
			 break;
		default:
			WebAssert.argTst(false, _mode.name()+" drag(..) only support for SelectType of ADD_FOOTPRINT or MOVE");
			break;
		}
	...
	private void end(WebPlotView pv) {
		...
		case ROTATE:
			...
			// _markerMap.get(_activeMarker).getDrawMan().redraw();
			 break;
		default:
			WebAssert.argTst(false, _mode.name()+ " end(..) only support for SelectType of ADD_FOOTPRINT or MOVE");
			break;
		}
		_doMove = false;
	}
	...
	private void updateData(OverlayMarker m, WebPlot plot, boolean drawHandles) {
		if (m.isReady()) {
			...
			if (drawHandles) {
				int size = 5; // size of the squared corners
	...
	private static ShapeDataObj.TextLocation convertTextLoc(OverlayMarker.Corner corner) {
		...
		case NW:
			return ShapeDataObj.TextLocation.CIRCLE_NW;
		case SE:
			return ShapeDataObj.TextLocation.CIRCLE_SE;
		case SW:
			return ShapeDataObj.TextLocation.CIRCLE_SW;
		}
		return ShapeDataObj.TextLocation.CIRCLE_NE;
	}
...
//		FootprintAsMarkers fp = new FootprintAsMarkers();
		...
		if (pv != null && pv.getPrimaryPlot() != null) {
			...
			_activeMarker.move(center, plot); // init marker
			updateData(_activeMarker, plot, true);
			_markerMap.get(_activeMarker).getDrawMan().redraw();
		}
	}
	...
	private class MarkerDrawing {
		...
		public void restoreColor() {
			if (_color != null)
				drawMan.setDefaultColor(_color);
		}
	...
	private class MarkerUICreator extends WebLayerItem.UICreator {
		...
		public Widget makeExtraUI(final WebLayerItem item) {
			...
			// Label remove = GwtUtil.makeLinkButton("remove", "remove marker",
			// new ClickHandler() {
			// public void onClick(ClickEvent event) {
		...
		private void updateTitle(WebLayerItem item, String title, String cStr, String id, String originalTitle) {
