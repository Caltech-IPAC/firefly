...
public class FootprintFactory  {
	...
	public enum INSTRUMENTS {
		...
		NIRISS(FOOTPRINT.JWST),
		NIS(NIRISS,""),
		
		NIRSPEC(FOOTPRINT.JWST),
		NRS(NIRSPEC,""),
		MSA(NIRSPEC,"");
		
		
		private String stc;
		private INSTRUMENTS parent = null;
		private FOOTPRINT fp;

		INSTRUMENTS(FOOTPRINT fp) {
			this.fp = fp;
		}

		INSTRUMENTS(INSTRUMENTS parent, String stcDef) {
			this.stc = stcDef;
			this.parent = parent;
			this.fp = parent.getFootprint();
		}
	...
	public static String getStcFromFootprint(FOOTPRINT fp){
		String stc = "";
...
+" POLYGON    0.00193333  -0.02526111  -0.02477500   0.00130833  -0.00116944   0.02586667   0.02621944  -0.00136389   0.00193333  -0.02526111";
	...
	private static HashMap<String,List<Double>> map;
	...
	public static String getFootprintStcStringDef(FOOTPRINT fp) {
		...
		case WFIRST:
			return wfirst;
		default:
			return getFootprintStcStringDef(FOOTPRINT.JWST);

		}

	}
	public List<Region> getFootprintAsRegionsFromString(String stc, WorldPt refCenter) {
		
		return getRegionFromStc(stc,refCenter);
	}
	/**
	 ...
	 * @return
	 */
	public List<Region> getFootprintAsRegions(FOOTPRINT fp, WorldPt refCenter) {
		
		String def = getFootprintStcStringDef(fp);
		List<Region> fpRegions = getRegionFromStc(def,refCenter);
		return fpRegions;
	}
	
	private List<Region> getRegionFromStc(String def, WorldPt refCenter) {
		...
		for (String string : keySet) {
			...
			if(string.startsWith("POL")){
				fpRegions.add(getPolygonLines(array,refCenter));
			}else if(string.startsWith("CIR")){
				fpRegions.add(getCircle(array,refCenter));
	...
	private Region getCircle(Double[] array, WorldPt refCenter) {
		WorldPt pt0 = new WorldPt(array[0], array[1]);
		WorldPt pt= VisUtil.calculatePosition(refCenter, pt0.getLon()*3600, pt0.getLat()*3600);
		double radius = array[2];
		return new RegionAnnulus(pt, new RegionValue(radius, RegionValue.Unit.DEGREE));

	}
	/**
	 * Expecting array length twice as lines of polygon
	 * @param xyCoords
	 * @return
	 */
	public static Region getPolygonLines(Double[] xyCoords, WorldPt refCenter) {
		WorldPt[] pts = new WorldPt[xyCoords.length / 2];
		for (int i = 0; i < xyCoords.length / 2; i++) {	
			//FIXME we have the region polygons based on 0,0 WC but we don't have it for any target - need geometry to translate it to any point in space.
			// Need to calc offset based on distances but newPos and calcPosition doesn't seems to be reliable.
//			double computeDistance = VisUtil.computeDistance(pt, center); //degree
//			double phi = VisUtil.getPositionAngle(pt,center);
//			WorldPt newPt = VisUtil.getNewPosition(pt.getLon(), pt.getLat(), computeDistance, phi);
//			pts[i] = new WorldPt(pt.getLon()+newPt.getLon(),pt.getLat()+newPt.getLat());
			WorldPt pt = new WorldPt(xyCoords[2 * i].doubleValue(), xyCoords[2 * i + 1].doubleValue());		
			pts[i]= VisUtil.calculatePosition(refCenter, pt.getLon()*3600, pt.getLat()*3600);
		}
		
		return new RegionLines(pts);
