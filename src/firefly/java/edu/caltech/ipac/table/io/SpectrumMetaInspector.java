...
public class SpectrumMetaInspector {
    ...
    /**
     * Look at the meta data for spectrum data model information. If found insert it into
     * the Data group.
     * <br>
     * spectrumHint only applies to non-FITS files
     * Guesser will look for columns the could possibly be spectral columns and build spectral data model
     * <br>
     * if fits file
     ...
     *    op 2: use guesser: always if no claim to be a specturm
     * <br>
     ...
     *    op 2. use guesser: if spectrumHint and no utype
     *
     *
     *
    ...
    public static void searchForSpectrum(DataGroup dg, BasicHDU<?> hdu, boolean spectrumHint) {
        ...
        // if I have reached this point in the code I am trying to build the spectral model
    ...
    private static String findColByUtypeOrNames(String[] possibleNames, List<DataType> dtAry, String utype) {
        ...
        return utypeCol!=null ? utypeCol : findCol(possibleNames,dtAry);
    }

    private static boolean hasCol(String[] possibleNames, List<DataType> dtAry, String utype, boolean useOnlyUtype) {
        String colName= useOnlyUtype ? findColByUtype(dtAry,utype) : findColByUtypeOrNames(possibleNames,dtAry,utype);
        return colName!=null;
    }
    ...
    private static void addSpectrumRef(DataGroup dg, List<DataType> dtAry, List<GroupInfo.RefInfo> list, boolean useOnlyUType) {
        ...
        String colName= useOnlyUType ? findColByUtype(dtAry,utype) : findColByUtypeOrNames(specColNames,dtAry,utype);
    ...
    private static void addErrorRef(DataGroup dg, List<DataType> dtAry, List<GroupInfo.RefInfo> list, boolean useOnlyUType) {
        ...
        String colName= useOnlyUType ? findColByUtype(dtAry,utype) : findColByUtypeOrNames(errColNames,dtAry,utype);
        if (colName==null) return;
    ...
    private static void addFluxRef(DataGroup dg, List<DataType> dtAry, List<GroupInfo.RefInfo> list, boolean useOnlyUType) {
        ...
        String colName= useOnlyUType ? findColByUtype(dtAry,utype) : findColByUtypeOrNames(fluxColNames,dtAry,utype);
        if (colName==null) return;
