...
public class Parser {
     ...
     */
    private HashMap<Variable,Variable> parsedVariables = null;
    ...
    public Set<Variable> getParsedVariables() {
        if (parsedVariables == null) return null;
            ...
            return parsedVariables.keySet();
        }
    }

    private void addParsedVariable(Variable var) {
        if (null == parsedVariables) {
            parsedVariables = new HashMap<Variable,Variable>();
        }
        parsedVariables.put(var, var);
    }
    ...
    private Token[] possibleInsertions(Token t) {
        ...
        for (String p1 : procs1) ts[i++] = new Token(Token.TT_WORD, 0, p1, t);

        for (String p2 : procs2) ts[i++] = new Token(Token.TT_WORD, 0, p2, t);
