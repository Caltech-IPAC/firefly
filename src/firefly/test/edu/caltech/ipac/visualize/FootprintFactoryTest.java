...
public class FootprintFactoryTest {
	...
	private String getFootprintString() {
		...
		/*			String jwst =	  " POLYGON    0.03794999  -0.17407720   0.03843333  -0.21384895   0.07796106  -0.21439047   0.07665551  -0.17417432   0.03794999  -0.17407720"
	...
	public void getFpString(){
		...
		List<Region> footprintRegions = footprintFactory.getFootprintAsRegionsFromString(getFootprintString(),
				new WorldPt(0, 0));//FootprintFactory.getFootprintAsRegions(fp, new WorldPt(0,0));
		...
		org.junit.Assert.assertTrue(c == 5); //2 circles are shape data obj and not footprint obj
	}

	@Test
	public void testSplit(){
		FOOTPRINT fp = FOOTPRINT.JWST;
		String[] split = FootprintFactory.getFootprintStcStringDef(fp).split("\\s");
		...
		if(fp.equals(FOOTPRINT.JWST)){
			...
			for (String string : keySet) {
				Assert.assertTrue("Wrong "+map.get(string).size() ,map.get(string).size() == 8);//should be x,y times 4, cuae rectangles are defined with 4 lines, 8 coordinates x,y
			}
			...
			Set<String> keySet = mapCir.keySet();
			for (String string : keySet) {
				Assert.assertTrue("Wrong "+mapCir.get(string).size() ,mapCir.get(string).size() == 3);//should be x,y,radius - 3 ekemetns
			}
		...
		for (FOOTPRINT footprint : values) {
			...
			footprintFactory.getFootprintAsRegions(footprint, refCenter);
		}
		...
		for (String string : keySet) {
			...
			for (int i = 0; i < array.length / 2; i++) {
				...
				System.out.println(pts[i].parse(pts[i].toString()));
			}
		}
	}
	@Test
	public void testCosineRa(){
		...
		String footprintStcStringDef = INSTRUMENTS.FGS1_FULL.getStc();
		Assert.assertTrue(footprintStcStringDef, polRA00.equals(footprintStcStringDef));
		
		List<Region> listref = footprintFactory.getFootprintAsRegionsFromString(polRA00, new WorldPt(0, 0));
		for (Region region : listref) {
			WorldPt[] ptAry = ((RegionLines) region).getPtAry();
			for (int i = 0; i < ptAry.length-1; i++) {
				System.out.println("Dist "+i+" "+ptAry[i].toString()+":"+VisUtil.computeDistance(ptAry[i], ptAry[i+1]));
			}
		}
		List<Region> list2 = footprintFactory.getFootprintAsRegionsFromString(polDEC40, new WorldPt(0, 0));
		...
		for (Region region2 : list2) {
			for (Region region : listref) {
				double lon = ((RegionLines) region).getPtAry()[0].getLon();//deg
				double lonRad = Math.toRadians(lon);
				double loncosdec = lonRad/Math.cos(Math.toRadians(dec));
				Assert.assertEquals(Math.toDegrees(loncosdec),
						((RegionLines) region2).getPtAry()[0].getLon(), 1e-3);
				Assert.assertEquals(((RegionLines) region2).getPtAry()[0].getLat()-
						((RegionLines) region).getPtAry()[0].getLat(),dec, 1e-3);
			}
		}
		dec=90;
		list2 = footprintFactory.getFootprintAsRegionsFromString(polDec90, new WorldPt(0, 0));
		for (Region region : list2) {
			WorldPt[] ptAry = ((RegionLines) region).getPtAry();
			for (int i = 0; i < ptAry.length-1; i++) {
				System.out.println("Dist "+i+" "+ptAry[i].toString()+":"+VisUtil.computeDistance(ptAry[i], ptAry[i+1]));
	...
	public void testConvertToNewReference(){
		...
		List<Region> reg = footprintFactory.getFootprintAsRegionsFromString(polRA00, new WorldPt(0,0));
