...
function creator() {
    ...
    return DrawLayer.makeDrawLayer( `${ID}-${idCnt}`, TYPE_ID, 'web grid',
        options , drawingDef, {});
...
function getLayerChanges(drawLayer, action) {
    ...
    var plot= primePlot(visRoot(),plotIdAry[0]);
               ...
               try {
                   switch (i) {
                       case 0:
                           ...
                           labels[offset] = labelFormat === 'hms' ? lon : CoordUtil.convertStringToLon(lon, csys).toFixed(1);
                           break;
                       case 1:
                           var lat = CoordUtil.convertLatToString(levels[i][j], csys);
                           labels[offset] = labelFormat === 'hms' ? lat : CoordUtil.convertStringToLat(lat, csys).toFixed(1);
                           break;
                   }

                 }catch (err) {
                        labels[offset] = levels[i][j]<1000? levels[i][j].toFixed(1):levels[i][j].toExponential(1).replace('e+', 'E');
                 }
