...
const activate= function(remoteChannel, extension, resultData) {
    ...
    if (extension.callback) {
        const cbObj= Object.keys(resultData).reduce((obj,key) => {
...
export const getPlotUIExtensionList= function(testPlotId) {
    const {extensionList}= flux.getState()[ExternalAccessCntlr.EXTERNAL_ACCESS_KEY];
    const retList= extensionList.filter((ext) => {
...
function getTwoPointSelectObj(plot,cc,attribute) {
    ...
    const {pt0,pt1}=plot.attributes[attribute];
    if (!pt0 && !pt1) return {};

    const data= {};
    const ipt0= cc.getImageCoords(pt0);
    ...
    const wpt0= cc.getWorldCoords(pt0);
    const wpt1= cc.getWorldCoords(pt1);
    if (wpt0 && wpt1) {
...
function getPointSelectObj(plot,cc) {
    ...
    const data= {};
    const ipt= cc.getImageCoords(pt);
