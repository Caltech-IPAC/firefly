 ...
 *
 */
export function jbposa( xlon1i, xlat1i, xlon0i, xlat0i) {
   var distd;
   var ipole, itest = 2, itesti;
   var xlon1,xlat1,xlon0,xlat0,xlon1r,xlon0r, xlat1r,xlat0r,costh;
   var ds,delar,sinp,cosp,p;
   var dellat,dellon,dist,dwork;
   var small = 1.0e-10, tfac = 36000000.0;
    ...
    if(Math.abs(xlat0i) > 89.9999) {
        itesti = Math.floor((90.0-Math.abs(xlat0i)) * tfac);
    ...
    if(Math.abs(xlat1i) > 89.9999) {
        itesti = Math.floor((90.0-Math.abs(xlat1i)) * tfac);
        if(Math.abs(itesti) <= itest) {
    ...
    if(Math.abs(distd) >= 90.0 ) return {ipf,distd,pa};
    if(xlat1==xlat0 && xlon1==xlon0) return {ipf,distd,pa};
    if(Math.abs(distd) < 1.667e-02) {
        ...
        distd = dist;
        ds = d2r * distd;
        if(dist <= small) return {ipf,distd,pa};
    }
...
/* compute position angle if possible */
      ...
      if(Math.abs(sinp) < 1.0e-09) {
         ...
         if(90.0-Math.abs(xlat0) >= distd) {
            if(xlat1 < xlat0) pa = 180.0;
         }
         else {
            dwork = Math.abs(xlon0 - xlon1);
            if(dwork > 180.0) dwork = Math.abs(dwork -360.0);
             if(xlat0 > 0.0) {                               // north pole:
 ...
 * @return xlonp,xlatp
 * @return   iflg indicates processing: 1 completely "normal"
 ...
 *
 */
export function jbelpt( xlonin, xlatin, distd, pain) {
    var xlonp,  xlatp, iflg;
    var donext = 1;
    var xlon0, xlat0, xlat0r, xlatpr=0.0, distr;
    var pa, par, dela=0.0, delar, part, b;
    var cosdp,sindp,sinpar;
    var small = 1.0e-09;
    ...
    var test = 89.99999;
    if(90.0-test > distd) test= 90.0 - (distd - 0.1*distd);
    iflg = 1;
    xlon0 = xlonin;
    xlat0 = xlatin;
    ...
    if (donext == 1) {
        if(Math.abs(xlat0) < small) {
            ...
            delar = Math.atan(1.0 / b);
            cosdp = part/Math.sin(delar);
            sindp = (Math.cos(distr) - b*Math.cos(xlat0r)*part) /Math.sin(xlat0r);
            if(Math.abs(cosdp) >= 1.0e-09) xlatpr = Math.atan2(sindp,cosdp);
            else xlatpr = Math.asin(sindp);
        }
    ...
    xlatp = r2d * xlatpr;
    if(Math.abs(xlatp) < test) return {iflg,xlonp,xlatp};

    if(Math.abs(xlatp) > 90.0) {
        if(xlatp < 90.0) xlatp = -(180.0 + xlatp);
        else xlatp = 180.0 - xlatp;
        xlonp = xlonp - 180.0;
        if(xlonp < 0.0) xlonp = xlonp + 360.0;
    }
