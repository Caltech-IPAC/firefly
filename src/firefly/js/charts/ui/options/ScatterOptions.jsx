...
import {getChartProps, getMinScatterGLRows, isFluxAxisOrder} from '../../ChartUtil.js';
...
import {Error_X, Error_Y, errorFieldKey, errorMinusFieldKey, errorTypeFieldKey} from './Errors.jsx';
import {getAppOptions} from '../../../core/AppDataCntlr.js';
import {getTblById} from '../../../tables/TableUtil.js';
import {PlotlyCS} from '../../Colorscale.js';
import {spectrumType} from '../../dataTypes/FireflySpectrum.js';
...
export function ScatterOptions({activeTrace:pActiveTrace, tbl_id:ptbl_id, chartId, groupKey}) {
    ...
    const showUseSpectrum = !isFluxAxisOrder(chartId) && dataType === spectrumType;
...
export function ScatterCommonOptions({activeTrace:pActiveTrace, tbl_id:ptbl_id, chartId, groupKey, fieldProps}) {
    ...
    const {Name, Color, Xreset, Yreset} = basicOptions({activeTrace, tbl_id, chartId, groupKey, fieldProps});
    const colValStats = getColValStats(tbl_id);
    const isOrder = isFluxAxisOrder(chartId);
...
export function submitChangesScatter({chartId, activeTrace, fields, tbl_id, renderTreeId}) {
    ...
    const {highlighted, selected, fireflyData, data} = getChartData(chartId);
    ...
    const cUseSpectrum =get(fireflyData, `${activeTrace}.useSpectrum`);
    const nUseSpectrum = get(fields, `fireflyData.${activeTrace}.useSpectrum`);
    if ( cUseSpectrum !== nUseSpectrum) {
        range(data.length).forEach((idx) => {
            if (idx !== activeTrace) {
                fields = updateSet(fields, [`fireflyData.${idx}.useSpectrum`], nUseSpectrum);
            }
        });
    }
...
export function scatterInputs ({activeTrace:pActiveTrace, tbl_id:ptbl_id, chartId, groupKey, fieldProps={}}) {
    ...
    return {
        ...
        Xmin: (props={}) => (<ColumnOrExpression fldPath = {`_tables.fireflyData.${activeTrace}.xMin`}
                                         initValue={get(mappings, `fireflyData.${activeTrace}.xMin`, '')}
        ...
        Xmax: (props={}) => (<ColumnOrExpression fldPath = {`_tables.fireflyData.${activeTrace}.xMax`}
                                         initValue={get(mappings, `fireflyData.${activeTrace}.xMax`, '')}
        ...
        Ymin: (props={}) => (<ColumnOrExpression fldPath = {`_tables.fireflyData.${activeTrace}.yMin`}
                                         initValue={get(mappings, `fireflyData.${activeTrace}.yMin`, '')}
                                         label = {'\u21A5:'}
                                         name = 'Lower Limit'
                                         nullAllowed = {true}
                                         colValStats={colValStats}
        ...
        Ymax: (props={}) => (<ColumnOrExpression fldPath = {`_tables.fireflyData.${activeTrace}.yMax`}
                                         initValue={get(mappings, `fireflyData.${activeTrace}.yMax`, '')}
                                         label = {'\u21A7:'}
                                         name = 'Upper Limit'
                                         nullAllowed={true}
                                         colValStats={colValStats}
        ...
        ColorScale: (props={}) => ( <ListBoxInputField fieldKey={`data.${activeTrace}.marker.colorscale`}
                                         ...
                                         nullAllowed={true}
                                          {...fieldProps} {...props}/>),
        UseSpectrum: (props={}) => (<CheckboxGroupInputField fieldKey={`fireflyData.${activeTrace}.useSpectrum`}
                                         initialState={{value: get(fireflyData, `${activeTrace}.useSpectrum`, 'true')}}
