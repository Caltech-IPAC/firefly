...
function updateChartData(chartId, traceNum, tablesource, action={}) {
    ...
    if (action.type === TABLE_HIGHLIGHT) {
        ...
        if (traceNum !== activeTrace && !isFluxAxisOrder(chartId)) return;
...
export function isFluxAxisOrder(chartId) {
    const {activeTrace=0, fireflyData} = getChartData(chartId) || {};
    return fireflyData?.[activeTrace]?.fluxAxisOrder;
}
...
export function getChartProps(chartId, tbl_id, activeTrace) {
    ...
    const hasXerrors = !isEmpty(get(data, `${activeTrace}.error_x.array`));
    const hasYerrors = !isEmpty(get(data, `${activeTrace}.error_y.array`));
    const hasYMax = get(fireflyData, `${activeTrace}.yMax`);
    const hasYMin = get(fireflyData, `${activeTrace}.yMin`);
    const hasXMax = get(fireflyData, `${activeTrace}.xMax`);
    const hasXMin = get(fireflyData, `${activeTrace}.xMin`);
    ...
    return {activeTrace, data, fireflyData, layout, fireflyLayout, tablesources, tablesource, mappings, tbl_id, type, noXY,
        dataType, noColor, isYNotNumeric, isXNotNumeric, xNoLog, yNoLog, color, multiTrace,
        hasXerrors, hasYerrors, hasYMax, hasYMin, hasXMax, hasXMin, spectralAxis, fluxAxis};
}
