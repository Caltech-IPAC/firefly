...
export function reducer(state=getInitState(), action={}) {
    switch (action.type) {
        ...
        case (LOAD_TBL_STATS)  :
        {
            ...
            const tblFilePath = get(TableUtil.getTblById(tblId), 'tableMeta.tblFilePath');

            return updateSet(state, tblId, {tblFilePath, isColStatsReady: false});
        }
